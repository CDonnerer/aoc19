...
def take_step(grid, current_position, new_position):
    ...
    grid[pos_y[0] : pos_y[1] + 1, pos_x[0] : pos_x[1] + 1] = True
...
def trace_wire(grid, central_port, wire):
    ...
    for step in wire:
        ...
        grid, current_position = take_step(grid, current_position, new_position)
...
def count_steps(grid, central_port, wire, target):
    ...
    for step in wire:
        ...
        if grid[target] == 1:
            ...
            grid_s, _ = take_step(grid_s, current_position, target)

            return grid.sum() - grid_s.sum()
